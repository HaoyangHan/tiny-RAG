...
from implementations.llm import OpenAIProvider, AnthropicProvider, OllamaProvider
...
```python
# rag-memo-core-lib/src/rag_memo_core_lib/implementations/llm/openai_provider.py
import openai
...
class RAGGenerator(Generator):
    ...
    async def initialize(self) -> None:
        ...
        if "vector_store_config" in self.config:
